...
// processes to communicate with and and manage the lifetime of a cmmd child
...
// message, shutdown begins.  This prevents cmmd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			cmmLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child cmmd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	cmmLog.Errorf("Failed to write to pipe: %v", err)
}
