...
import (
	...
	"github.com/CommerciumBlockchain/cmmd/blockchain/indexers"
	...
	"github.com/CommerciumBlockchain/cmmd/mining"
...
const (
	...
	MinHighPriority = cmmutil.AtomsPerCoin * 144.0 / 250
	...
	// CMM/kB, this results in a maximum allowed high fee of 1 CMM/kB.
...
type Config struct {
	...
	FetchUtxoView func(*cmmutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*cmmutil.Block, error)
	...
	// utxo view.
	CalcSequenceLock func(*cmmutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*cmmutil.Tx
	orphansByPrev map[chainhash.Hash]map[chainhash.Hash]*cmmutil.Tx
	outpoints     map[wire.OutPoint]*cmmutil.Tx
...
func (mp *TxPool) insertVote(ssgen *cmmutil.Tx) error {
	msgTx := ssgen.MsgTx()
	...
	vote := cmmutil.IsFlagSet16(voteBits, cmmutil.BlockValid)
...
// TODO Pruning of the votes map Commercium
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *cmmutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		...
		if _, exists := mp.orphansByPrev[originTxHash]; !exists {
			mp.orphansByPrev[originTxHash] =
				make(map[chainhash.Hash]*cmmutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *cmmutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *cmmutil.Tx, removeRedeemers bool) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *cmmutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *cmmutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *cmmutil.Tx, txType stake.TxType) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *cmmutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash, includeRecentBlock bool) (*cmmutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
	...
	// For Commercium, the latest block is considered "unconfirmed"
...
// It should also set the cmmutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *cmmutil.Tx, isNew, rateLimit, allowHighFees bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *cmmutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(hash *chainhash.Hash) []*cmmutil.Tx {
	var acceptedTxns []*cmmutil.Tx
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *cmmutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*cmmutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*cmmutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*cmmjson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*cmmjson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &cmmjson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Fee:              cmmutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*cmmutil.Tx),
		orphansByPrev: make(map[chainhash.Hash]map[chainhash.Hash]*cmmutil.Tx),
		outpoints:     make(map[wire.OutPoint]*cmmutil.Tx),
