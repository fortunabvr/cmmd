...
import (
	...
	"github.com/CommerciumBlockchain/cmmd/blockchain/chaingen"
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := cmmutil.Hash160(redeemScript)
...
// subsidy.
func additionalCoinbasePoW(amount cmmutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee cmmutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	// tests.
	g, err := chaingen.MakeGenerator(simNetParams, nil)
	if err != nil {
		return nil, err
	}
	...
	// vector is not possible in Commercium since the block size is included in
	...
	// Commercium enforces the coinbase includes the block height to which it
	...
	g.NextBlock("bcb9", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = cmmutil.MaxAmount + 1
	})
	rejected(blockchain.ErrBadTxOutValue)
	...
	g.NextBlock("bcb10", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = cmmutil.MaxAmount
