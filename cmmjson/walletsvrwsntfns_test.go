...
func TestWalletSvrWsNtfns(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "accountbalance",
			newNtfn: func() (interface{}, error) {
				return cmmjson.NewCmd("accountbalance", "acct", 1.25, true)
			},
			staticNtfn: func() interface{} {
				return cmmjson.NewAccountBalanceNtfn("acct", 1.25, true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"accountbalance","params":["acct",1.25,true],"id":null}`,
			unmarshalled: &cmmjson.AccountBalanceNtfn{
				Account:   "acct",
				Balance:   1.25,
				Confirmed: true,
			},
		},
		{
			name: "cmmdconnected",
			newNtfn: func() (interface{}, error) {
				return cmmjson.NewCmd("cmmdconnected", true)
			},
			staticNtfn: func() interface{} {
				return cmmjson.NewBtcdConnectedNtfn(true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"cmmdconnected","params":[true],"id":null}`,
			unmarshalled: &cmmjson.BtcdConnectedNtfn{
				Connected: true,
			},
		},
		{
			...
			newNtfn: func() (interface{}, error) {
				return cmmjson.NewCmd("walletlockstate", true)
			},
			staticNtfn: func() interface{} {
				return cmmjson.NewWalletLockStateNtfn(true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"walletlockstate","params":[true],"id":null}`,
			unmarshalled: &cmmjson.WalletLockStateNtfn{
				Locked: true,
			},
		},
		{
			name: "newtx",
			newNtfn: func() (interface{}, error) {
				return cmmjson.NewCmd("newtx", "acct", `{"account":"acct","address":"1Address","category":"send","amount":1.5,"fee":0.0001,"confirmations":1,"txid":"456","walletconflicts":[],"time":12345678,"timereceived":12345876,"vout":789,"otheraccount":"otheracct"}`)
			},
			staticNtfn: func() interface{} {
				result := cmmjson.ListTransactionsResult{
					Account:         "acct",
					Address:         "1Address",
					Category:        "send",
					Amount:          1.5,
					Fee:             cmmjson.Float64(0.0001),
					Confirmations:   1,
					TxID:            "456",
				...
				return cmmjson.NewNewTxNtfn("acct", result)
			},
			...
			unmarshalled: &cmmjson.NewTxNtfn{
				Account: "acct",
				Details: cmmjson.ListTransactionsResult{
					Account:         "acct",
					Address:         "1Address",
					Category:        "send",
					Amount:          1.5,
