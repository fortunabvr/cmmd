...
const (
	...
	unusableFlags = cmmjson.UFWebsocketOnly | cmmjson.UFNotification
)

var (
	cmmdHomeDir           = cmmutil.AppDataDir("cmmd", false)
	cmmctlHomeDir         = cmmutil.AppDataDir("cmmctl", false)
	cmmwalletHomeDir      = cmmutil.AppDataDir("cmmwallet", false)
	defaultConfigFile      = filepath.Join(cmmctlHomeDir, "cmmctl.conf")
	defaultRPCServer       = "localhost"
	defaultWalletRPCServer = "localhost"
	defaultRPCCertFile     = filepath.Join(cmmdHomeDir, "rpc.cert")
	defaultWalletCertFile  = filepath.Join(cmmwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := cmmjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := cmmjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := cmmjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&cmmjson.UFWalletOnly != 0 {
...
// config defines the configuration options for cmmcmmctl.
//
...
// For this it tries to read the cmmd config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing cmmd conf file at the default
	// path to extract the details from.
	cmmdConfigPath := filepath.Join(cmmdHomeDir, "cmmd.conf")
	if !fileExists(cmmdConfigPath) {
		return nil
	}

	// Read cmmd.conf from its default path
	cmmdConfigFile, err := os.Open(cmmdConfigPath)
	if err != nil {
		return err
	}
	defer cmmdConfigFile.Close()
	content, err := ioutil.ReadAll(cmmdConfigFile)
	if err != nil {
		return err
	}
