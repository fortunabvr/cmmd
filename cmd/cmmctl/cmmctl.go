...
func main() {
	...
	method := args[0]
	usageFlags, err := cmmjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := cmmjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// cmmjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(cmmjson.Error); ok {
		...
		// The error is not a cmmjson.Error and this really should not
	...
	marshalledJSON, err := cmmjson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
