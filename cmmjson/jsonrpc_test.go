...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if cmmjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *cmmjson.RPCError {
				return cmmjson.NewRPCError(cmmjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := cmmjson.MarshalResponse("1.0", testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := cmmjson.NewRequest("1.0", nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := cmmjson.Error{Code: cmmjson.ErrInvalidType}
	_, err = cmmjson.MarshalResponse("", make(chan int), nil, nil)
	if jerr, ok := err.(cmmjson.Error); !ok || jerr.Code != wantErr.Code {
	...
	// can't be marshalled.
	_, err = cmmjson.MarshalResponse("1.0", 1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			cmmjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			cmmjson.ErrRPCMethodNotFound,
