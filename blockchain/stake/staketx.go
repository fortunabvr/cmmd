...
func AddrFromSStxPkScrCommitment(pkScript []byte,
	params *chaincfg.Params) (cmmutil.Address, error) {
	...
	if isP2SH {
		addr, err = cmmutil.NewAddressScriptHashFromHash(hashBytes, params)
	} else {
		addr, err = cmmutil.NewAddressPubKeyHash(hashBytes, params,
...
// ticket commitment pkScript.
func AmountFromSStxPkScrCommitment(pkScript []byte) (cmmutil.Amount, error) {
	if len(pkScript) < SStxPKHMinOutSize {
	...
	return cmmutil.Amount(binary.LittleEndian.Uint64(amtEncoded)), nil
}
