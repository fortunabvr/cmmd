...
// FileContents is a string containing the commented example config for cmmd.
...
; The default is ~/.cmmd/data on POSIX OSes, $LOCALAPPDATA/Cmmd/data on Windows,
; ~/Library/Application Support/cmmd/data on macOS, and $homed/cmmd/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.cmmd/data                           ; Unix
; datadir=$LOCALAPPDATA/Cmmd/data                 ; Windows
; datadir=~/Library/Application Support/Cmmd/data ; macOS
...
; line.  cmmd will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, cmmd will query DNS
...
; Disable DNS seeding for peers.  By default, when cmmd starts, it will use
...
; which is used to control and query information from a running cmmd process.
;
...
; log level for individual subsystems.  Use cmmd --debuglevel=show to list
