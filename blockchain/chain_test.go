...
func TestConvertToNewFormat(t *testing.T) {
	...
	ofilename := filepath.Join("testdata/", "blocks0to168.cmmd.json.gz")
	...
	for decoder.More() {
		...
		merkles := BuildMerkleTreeStore(cmmutil.NewBlock(&bl.MsgBlock).Transactions())
...
func TestBlockchainFunctions(t *testing.T) {
	...
	// Load up the rest of the blocks up to HEAD~1.
	filename := filepath.Join("testdata/", "blocks0to168.cmmd.json.gz")
	fi, err := os.Open(filename)
	if err != nil {
		t.Errorf("Unable to open %s: %v", filename, err)
	}
	...
	for i := 1; i <= 168; i++ {
		...
		bl := cmmutil.NewBlock(&jsbl.MsgBlock)
	...
	expectedVal := cmmutil.Amount(3495091704)
	if val != expectedVal {
		t.Errorf("Failed to get correct result for ticket pool value; "+
			"want %v, got %v", expectedVal, val)
	}

	a, _ := cmmutil.DecodeAddress("SsbKpMkPnadDcZFFZqRPY8nvdFagrktKuzB")
...
func TestForceHeadReorg(t *testing.T) {
	...
	accepted := func() {
		...
		block := cmmutil.NewBlock(msgBlock)
