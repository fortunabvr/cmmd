...
func TestEncodeConcatenatedHashes(t *testing.T) {
	// Input Hash slice. Data taken from Commercium's first three mainnet blocks.
	...
	for j := 0; j < len(hashSlice)+1; j++ {
		...
		// Encode to string
		concatenated := cmmjson.EncodeConcatenatedHashes(hashSlice[:j])
		// Verify output
...
func TestDecodeConcatenatedHashes(t *testing.T) {
	// Test data taken from Commercium's first three mainnet blocks
	...
	decodedHashes, err := cmmjson.DecodeConcatenatedHashes(concatenatedHashes)
	if err != nil {
...
func TestEncodeConcatenatedVoteBits(t *testing.T) {
	...
	encodedResults, err := cmmjson.EncodeConcatenatedVoteBits(testVbs)
	if err != nil {
	...
	_, err = cmmjson.EncodeConcatenatedVoteBits(testVbs)
	if err == nil {
...
func TestDecodeConcatenatedVoteBits(t *testing.T) {
	...
	decodedSlice, err :=
		cmmjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err != nil {
	...
	_, err = cmmjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err == nil {
...
func TestInvalidDecodeConcatenatedHashes(t *testing.T) {
	testStrings := []struct {
	...
	}{
		{
			...
			cmmjson.RPCError{
				Code: cmmjson.ErrRPCDecodeHexString,
			},
		}, {
			// invalid length
