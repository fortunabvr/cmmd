...
// TestCmmdCmds tests all of the btcd extended commands marshal and unmarshal
// into valid results include handling of optional fields being omitted in the
// marshalled command, while optional fields with defaults have the default
// assigned on unmarshalled commands.
func TestCmmdCmds(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			newCmd: func() (interface{}, error) {
				return cmmjson.NewCmd("getstakeversions", "deadbeef", 1)
			},
			staticCmd: func() interface{} {
				return cmmjson.NewGetStakeVersionsCmd("deadbeef", 1)
			},
			marshalled: `{"jsonrpc":"1.0","method":"getstakeversions","params":["deadbeef",1],"id":1}`,
			unmarshalled: &cmmjson.GetStakeVersionsCmd{
				Hash:  "deadbeef",
				Count: 1,
			},
		},
		{
			name: "getvoteinfo",
			newCmd: func() (interface{}, error) {
				return cmmjson.NewCmd("getvoteinfo", 1)
			},
			staticCmd: func() interface{} {
				return cmmjson.NewGetVoteInfoCmd(1)
			},
			marshalled: `{"jsonrpc":"1.0","method":"getvoteinfo","params":[1],"id":1}`,
			unmarshalled: &cmmjson.GetVoteInfoCmd{
				Version: 1,
			},
		},
	}
