...
import (
	...
	"github.com/CommerciumBlockchain/cmmd/peer"
...
var (
	...
	// ourselves to other Commercium peers.
	userAgentName = "cmmd"
...
// broadcastMsg provides the ability to house a Commercium message to be broadcast
...
// server provides a Commercium server for handling communications to and from
// Commercium peers.
type server struct {
...
func (sp *serverPeer) OnTx(p *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a cmmutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := cmmutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a cmmutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := cmmutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) AnnounceNewTransactions(newTxs []*cmmutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices, cmmdLookup, func(addrs []*wire.NetAddress) {
...
// AddedNodeInfo returns an array of cmmjson.GetAddedNodeInfoResult structures
...
// newServer returns a new cmmd server configured to listen on addr for the
// Commercium network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, cmmdLookup)
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The cmmdLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := cmmdLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
