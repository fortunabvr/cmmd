...
func TestAddresses(t *testing.T) {
	tests := []struct {
		...
		encoded string
		valid   bool
		result  cmmutil.Address
		f       func() (cmmutil.Address, error)
		net     *chaincfg.Params
	}{
		// Positive P2PKH tests.
		{
			...
			valid:   true,
			result: cmmutil.TstAddressPubKeyHash(
				[ripemd160.Size]byte{
			...
			f: func() (cmmutil.Address, error) {
				pkHash := []byte{
					0x4d, 0x02, 0x1b, 0xca, 0xad, 0x14, 0xe2, 0x4d, 0xe9, 0x04,
					0x78, 0xe7, 0x69, 0x52, 0xe3, 0xac, 0xdb, 0xd8, 0x08, 0x36}
				return cmmutil.NewAddressPubKeyHash(pkHash,
		...
		// Positive P2SH tests.
		{
			...
			valid:   true,
			result: cmmutil.TstAddressScriptHash(
				[ripemd160.Size]byte{
			...
			f: func() (cmmutil.Address, error) {
				...
				return cmmutil.NewAddressScriptHash(txscript, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (cmmutil.Address, error) {
				hash := []byte{
					0xc7, 0xda, 0x50, 0x95, 0x68, 0x34, 0x36, 0xf4, 0x43, 0x5f,
					0xc4, 0xe7, 0x16, 0x3d, 0xca, 0xfd, 0xa1, 0xa2, 0xd0, 0x07}
				return cmmutil.NewAddressScriptHashFromHash(hash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (cmmutil.Address, error) {
				hash := []byte{
					0x36, 0xc1, 0xca, 0x10, 0xa8, 0xa6, 0xa4, 0xb5, 0xd4, 0x20,
					0x4a, 0xc9, 0x70, 0x85, 0x39, 0x79, 0x90, 0x3a, 0xa2, 0x84}
				return cmmutil.NewAddressScriptHashFromHash(hash, &chaincfg.TestNetParams)
			},
			net: &chaincfg.TestNetParams,
		},
		...
		// Positive P2PK tests.
		{
			...
			valid:   true,
			result: cmmutil.TstAddressPubKey(
				[]byte{
					...
					0x0f, 0xe2, 0xed},
				cmmutil.PKFCompressed, chaincfg.MainNetParams.PubKeyHashAddrID),
			f: func() (cmmutil.Address, error) {
				serializedPubKey := []byte{
					...
					0x52, 0xe7, 0xfe, 0xf0, 0xea, 0x6d, 0xa1, 0x43, 0xa0, 0x2b,
					0x0f, 0xe2, 0xed}
				return cmmutil.NewAddressSecpPubKey(serializedPubKey, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
		...
		},
		{
			...
			result: cmmutil.TstAddressPubKey(
				...
				cmmutil.PKFUncompressed, chaincfg.MainNetParams.PubKeyHashAddrID),
			f: func() (cmmutil.Address, error) {
				serializedPubKey := []byte{
					0x04, 0x64, 0xc4, 0x46, 0x53, 0xd6, 0x56, 0x7e, 0xff, 0x57,
					0x53, 0xc5, 0xd2, 0x4a, 0x68, 0x2d, 0xdc, 0x2b, 0x2c, 0xad,
		...
		},
		{
			...
			result: cmmutil.TstAddressPubKey(
				[]byte{
					...
					0x4b, 0xe0, 0x6e},
				cmmutil.PKFCompressed, chaincfg.TestNetParams.PubKeyHashAddrID),
			f: func() (cmmutil.Address, error) {
				serializedPubKey := []byte{
					0x02, 0x6a, 0x40, 0xc4, 0x03, 0xe7, 0x46, 0x70, 0xc4, 0xde,
					0x76, 0x56, 0xa0, 0x9c, 0xaa, 0x23, 0x53, 0xd4, 0xb3, 0x83,
					0xa9, 0xce, 0x66, 0xee, 0xf5, 0x1e, 0x12, 0x20, 0xea, 0xcf,
					0x4b, 0xe0, 0x6e}
				return cmmutil.NewAddressSecpPubKey(serializedPubKey, &chaincfg.TestNetParams)
			},
			net: &chaincfg.TestNetParams,
		},
		{
		...
		},
		{
			...
			result: cmmutil.TstAddressPubKey(
				...
				cmmutil.PKFUncompressed, chaincfg.TestNetParams.PubKeyHashAddrID),
			f: func() (cmmutil.Address, error) {
				serializedPubKey := []byte{
					0x04, 0x6a, 0x40, 0xc4, 0x03, 0xe7, 0x46, 0x70, 0xc4, 0xde,
					0x76, 0x56, 0xa0, 0x9c, 0xaa, 0x23, 0x53, 0xd4, 0xb3, 0x83,
	...
	for _, test := range tests {
		...
		decoded, err := cmmutil.DecodeAddress(test.addr)
		...
		if err == nil {
			...
			case *cmmutil.AddressPubKeyHash:
				saddr = cmmutil.TstAddressSAddr(encoded)
