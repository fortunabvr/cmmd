...
func TestEncodeDecodeUncompressedWIF(t *testing.T) {
	...
	wif1, err := cmmutil.NewUncompressedWIF(priv1, &chaincfg.MainNetParams)
	if err != nil {
		t.Fatal(err)
	}
	wif2, err := cmmutil.NewUncompressedWIF(priv2, &chaincfg.TestNetParams)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range tests {
		...
		// structure.
		w, err := cmmutil.DecodeWIF(test.encoded)
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestEncodeDecodeWIF(t *testing.T) {
	...
	for _, ecType := range suites {
		...
		wif1, err := cmmutil.NewWIF(priv1, &chaincfg.MainNetParams, ecType)
		if err != nil {
			t.Fatal(err)
		}
		wif2, err := cmmutil.NewWIF(priv2, &chaincfg.TestNetParams, ecType)
		if err != nil {
			t.Fatal(err)
		}
