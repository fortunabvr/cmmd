...
type NotificationHandlers struct {
	...
	// such as cmmwallet.
	OnAccountBalance func(account string, balance cmmutil.Amount, confirmed bool)
	...
	// This will only be available when client is connected to a wallet
	// server such as cmmwallet.
	OnTicketsPurchased func(TxHash *chainhash.Hash, amount cmmutil.Amount)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case cmmjson.BlockConnectedNtfnMethod:
	...
	// OnBlockDisconnected
	case cmmjson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case cmmjson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case cmmjson.ReorganizationNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWinningTickets
	case cmmjson.WinningTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case cmmjson.SpentAndMissedTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnNewTickets
	case cmmjson.NewTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnStakeDifficulty
	case cmmjson.StakeDifficultyNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case cmmjson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case cmmjson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBtcdConnected
	case cmmjson.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		...
		if err != nil {
			log.Warnf("Received invalid cmmd connected "+
				"notification: %v", err)
			return
		}
	...
	// OnAccountBalance
	case cmmjson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTicketPurchased:
	case cmmjson.TicketPurchasedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnVotesCreated:
	case cmmjson.VoteCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case cmmjson.RevocationCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case cmmjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := cmmutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*cmmjson.TxRawResult,
	error) {
...
// parseBtcdConnectedNtfnParams parses out the connection status of cmmd
// and cmmwallet from the parameters of a btcdconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance cmmutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := cmmutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func parseTicketPurchasedNtfnParams(params []json.RawMessage) (txHash *chainhash.Hash,
	amount cmmutil.Amount, err error) {
	...
	return thHash, cmmutil.Amount(amt), nil
}
...
//
// NOTE: This is a cmmd extension and requires a websocket connection.
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := cmmjson.NewNotifyBlocksCmd()
...
func (c *Client) NotifyWinningTicketsAsync() FutureNotifyWinningTicketsResult {
	...
	cmd := cmmjson.NewNotifyWinningTicketsCmd()
...
func (c *Client) NotifySpentAndMissedTicketsAsync() FutureNotifySpentAndMissedTicketsResult {
	...
	cmd := cmmjson.NewNotifySpentAndMissedTicketsCmd()
...
func (c *Client) NotifyNewTicketsAsync() FutureNotifyNewTicketsResult {
	...
	cmd := cmmjson.NewNotifyNewTicketsCmd()
...
func (c *Client) NotifyStakeDifficultyAsync() FutureNotifyStakeDifficultyResult {
	...
	cmd := cmmjson.NewNotifyStakeDifficultyCmd()
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := cmmjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a cmmd extension and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []cmmutil.Address,
	...
	outPointObjects := make([]cmmjson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = cmmjson.OutPoint{
	...
	cmd := cmmjson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a cmmd extension and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []cmmutil.Address, outPoints []wire.OutPoint) error {
