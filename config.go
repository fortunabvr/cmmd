...
import (
	...
	_ "github.com/CommerciumBlockchain/cmmd/database/ffldb"
	"github.com/CommerciumBlockchain/cmmd/cmmutil"
	"github.com/CommerciumBlockchain/cmmd/mempool"
	"github.com/CommerciumBlockchain/cmmd/sampleconfig"
...
const (
	defaultConfigFilename        = "cmmd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "cmmd.log"
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = cmmutil.AppDataDir("cmmd", false)
...
// config defines the configuration options for cmmd.
//
...
type config struct {
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in CMM/kB to be considered a non-zero fee."`
...
// createDefaultConfig copies the file sample-cmmd.conf to the given destination path,
...
//
// The above results in cmmd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for cmmd if specified. Since the home
	...
	cfg.minRelayTxFee, err = cmmutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]cmmutil.Address, 0, len(cfg.GetWorkKeys)+
		len(cfg.MiningAddrs))
	for _, strAddr := range cfg.GetWorkKeys {
		addr, err := cmmutil.DecodeAddress(strAddr)
		if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			cmmLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		cmmLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// cmmdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func cmmdDial(network, addr string) (net.Conn, error) {
...
// cmmdLookup returns the correct DNS lookup function to use depending on the
...
func cmmdLookup(host string) ([]net.IP, error) {
