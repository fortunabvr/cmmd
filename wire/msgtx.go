...
// OutPoint defines a Commercium data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new Commercium transaction outpoint point with the
...
// TxIn defines a Commercium transaction input.
type TxIn struct {
	// Non-witness
...
// NewTxIn returns a new Commercium transaction input with the provided
...
// TxOut defines a Commercium transaction output.
type TxOut struct {
	Value    int64
	Version  uint16
	PkScript []byte
...
// NewTxOut returns a new Commercium transaction output with the provided
...
// MsgTx implements the Message interface and represents a Commercium tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, serType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in Commercium
...
// BtcDecode decodes r using the Commercium protocol encoding into the receiver.
...
// in that BtcDecode decodes from the Commercium wire protocol as it was sent
...
// BtcEncode encodes the receiver to w using the Commercium protocol encoding.
// This is part of the Message interface implementation.
...
// encodes the transaction to the Commercium wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new Commercium tx message that conforms to the Message
...
// WriteOutPoint encodes op to the Commercium protocol encoding for an OutPoint
// to w.
...
// writeTxInPrefixs encodes ti to the Commercium protocol encoding for a transaction
...
// writeTxWitness encodes ti to the Commercium protocol encoding for a transaction
...
// writeTxInWitnessSigning encodes ti to the Commercium protocol encoding for a
...
// writeTxInWitnessValueSigning encodes ti to the Commercium protocol encoding for a
...
// writeTxOut encodes to into the Commercium protocol encoding for a transaction
// output (TxOut) to w.
