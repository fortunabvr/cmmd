...
/*
Package rpcclient implements a websocket-enabled Commercium JSON-RPC client.

Overview

This client provides a robust and easy to use client for interfacing
with a Commercium RPC server that uses a mostly btcd/bitcoin core
style Commercium JSON-RPC API.  This client has been tested with cmmd
(https://github.com/CommerciumBlockchain/cmmd) and cmmwallet
(https://github.com/CommerciumBlockchain/cmmwallet).

In addition to the compatible standard HTTP POST JSON-RPC API, cmmd and
cmmwallet provide a websocket interface that is more efficient than the standard
...
cmmd or cmmwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by cmmd and
cmmwallet only uses a single connection that remains open and allows
...
All notifications provided by cmmd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by cmmd (and
cmmwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that cmmd intentionally separates the wallet
functionality into a separate process named cmmwallet.  This means if you are
connected to the cmmd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, cmmwallet provides pass through treatment
...
detected by type asserting the error in a *cmmjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*cmmjson.RPCError); ok {
  		switch jerr.Code {
  		case cmmjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - cmmdwebsockets
   ...
   block count
 - cmmwalletwebsockets
   Connects to a cmmwallet RPC server using TLS-secured websockets, registers
