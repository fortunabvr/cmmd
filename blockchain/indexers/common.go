...
type Indexer interface {
	...
	ConnectBlock(dbTx database.Tx, block, parent *cmmutil.Block, view *blockchain.UtxoViewpoint) error

	// DisconnectBlock is invoked when the index manager is notified that a
	// block has been disconnected from the main chain.
	DisconnectBlock(dbTx database.Tx, block, parent *cmmutil.Block, view *blockchain.UtxoViewpoint) error
}
...
// be considered valid.
func approvesParent(block *cmmutil.Block) bool {
	return cmmutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
		cmmutil.BlockValid)
}
