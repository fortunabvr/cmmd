...
import (
	...
	"github.com/CommerciumBlockchain/cmmd/hdkeychain"
	"github.com/CommerciumBlockchain/cmmd/rpcclient"
...
type memWallet struct {
	...
	addrs map[uint32]cmmutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]cmmutil.Address)
...
// SetRPCClient saves the passed rpc connection to cmmd as the wallet's
...
func (m *memWallet) IngestBlock(header []byte, filteredTxns [][]byte) {
	...
	txns := make([]*cmmutil.Tx, 0, len(filteredTxns))
	for _, txBytes := range filteredTxns {
		tx, err := cmmutil.NewTxFromBytes(txBytes)
		if err != nil {
			panic(err)
		}
		txns = append(txns, tx)
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash, isCoinbase bool, undo *undoEntry) {
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          cmmutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (cmmutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []cmmutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt cmmutil.Amount, feeRate cmmutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := cmmutil.Amount(txSize * int(feeRate))
...
// in atoms-per-byte.
func (m *memWallet) SendOutputs(outputs []*wire.TxOut, feeRate cmmutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate cmmutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt cmmutil.Amount
	for _, output := range outputs {
		outputAmt += cmmutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() cmmutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance cmmutil.Amount
...
func keyToAddr(key chainec.PrivateKey, net *chaincfg.Params) (cmmutil.Address, error) {
	pubKey := chainec.Secp256k1.NewPublicKey(key.Public())
	serializedKey := pubKey.SerializeCompressed()
	pubKeyAddr, err := cmmutil.NewAddressSecpPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
