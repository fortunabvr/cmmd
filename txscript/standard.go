...
// valid SStxs.
func PayToSStx(addr cmmutil.Address) ([]byte, error) {
	if addr == nil {
...
// valid SStxs.
func PayToSStxChange(addr cmmutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// valid SSGen txs.
func PayToSSGen(addr cmmutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// valid SSRtx.
func PayToSSRtx(addr cmmutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// an SStx.
func GenerateSStxAddrPush(addr cmmutil.Address, amount cmmutil.Amount,
...
// specified address.
func PayToAddrScript(addr cmmutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *cmmutil.AddressPubKeyHash:
		if addr == nil {
			return nil, ErrUnsupportedAddress
...
func MultiSigScript(pubkeys []*cmmutil.AddressSecpPubKey, nrequired int) ([]byte,
	error) {
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []cmmutil.Address, int, error) {
	if version != DefaultScriptVersion {
		return NonStandardTy, nil, 0, fmt.Errorf("invalid script version")
	}

	var addrs []cmmutil.Address
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := cmmutil.NewAddressPubKeyHash(pops[2].data,
	...
	case PubKeyTy:
		...
		if err == nil {
			addr, err := cmmutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	case PubkeyAltTy:
		...
		requiredSigs = 1
		suite, _ := ExtractPkScriptAltSigType(pkScript)
		var addr cmmutil.Address
		err := fmt.Errorf("invalid signature suite for alt sig")
		switch suite {
		case chainec.ECTypeEdwards:
			addr, err = cmmutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case chainec.ECTypeSecSchnorr:
			addr, err = cmmutil.NewAddressSecSchnorrPubKey(pops[0].data,
				chainParams)
		}
		if err == nil {
			addrs = append(addrs, addr)
	...
	case StakeSubmissionTy:
		...
		var localAddrs []cmmutil.Address
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := cmmutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]cmmutil.Address, 0, numPubKeys)
...
//
// NOTE: Atomic swaps are not considered standard script types by the cmmd
