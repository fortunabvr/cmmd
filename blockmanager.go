...
// blockMsg packages a Commercium block message and the peer it came from together
...
// invMsg packages a Commercium inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a Commercium headers message and the peer it came from
...
// txMsg packages a Commercium tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *cmmutil.Tx
	peer *serverPeer
...
func (b *blockManager) checkBlockForHiddenVotes(block *cmmutil.Block) {
	var votesFromBlock []*cmmutil.Tx
	...
	var newVotes []*cmmutil.Tx
	var oldTickets []*cmmutil.Tx
	var oldRevocations []*cmmutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.server.chainParams.TicketsPerBlock))
	if template != nil {
		templateBlock := cmmutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*cmmutil.Tx, 0,
	...
	var updatedTxTreeRegular []*cmmutil.Tx
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected: // A block has been connected to the main block chain.
		blockSlice, ok := notification.Data.([]*cmmutil.Block)
		if !ok {
		...
		// below.
		txTreeRegularValid := cmmutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
			cmmutil.BlockValid)
...
// queue.
func (b *blockManager) QueueTx(tx *cmmutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *cmmutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *cmmutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *cmmutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*cmmutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (cmmutil.Amount, error) {
...
//
// This function is safe for concurrent access.
func (b *blockManager) SetMiningAddr(addr *cmmutil.Address) {
...
func (b *blockManager) GetMiningAddr() (cmmutil.Address, error) {
...
// newBlockManager returns a new Commercium block manager.
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		cmmLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		cmmLog.Infof("Creating block database in memory.")
	...
	cmmLog.Infof("Loading block database from '%s'", dbPath)
	...
	cmmLog.Info("Block database loaded")
	return db, nil
}
