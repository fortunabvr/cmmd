...
// Tx returns a wrapped transaction (cmmutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (cmmutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (cmmutil.Tx) for all
...
// instead provides easy access to wrapped versions (cmmutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (cmmutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (cmmwire.MsgTx) in the underlying wire.MsgBlock, however it
