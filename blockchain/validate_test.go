...
func recalculateMsgBlockMerkleRootsSize(msgBlock *wire.MsgBlock) {
	tempBlock := cmmutil.NewBlock(msgBlock)
...
func TestBlockValidationRules(t *testing.T) {
	...
	for i := 1; i < stakeEarlyTest; i++ {
		bl, err := cmmutil.NewBlockFromBytes(blockChain[int64(i)])
		if err != nil {
	...
	b142test := cmmutil.NewBlock(earlySSGen142)
	...
	block153, err := cmmutil.NewBlockFromBytes(blockChain[int64(testsIdx1)])
	if err != nil {
		t.Errorf("NewBlockFromBytes error: %v", err.Error())
	}
	...
	b153test := cmmutil.NewBlock(missingParent153)
	...
	b153test = cmmutil.NewBlock(ssrtxPayeesMismatch153)
	...
	b153test = cmmutil.NewBlock(badSSRtxPayee153)
	...
	b153test = cmmutil.NewBlock(badSSRtx153)
	...
	b153test = cmmutil.NewBlock(block153MsgBlock)
	...
	b154test := cmmutil.NewBlock(block154MsgBlock)
	...
	b154test = cmmutil.NewBlock(nonChosenTicket154)
	...
	b154test = cmmutil.NewBlock(spendTaggedIn154)
	...
	b154test = cmmutil.NewBlock(spendTaggedOut154)
	...
	b154test = cmmutil.NewBlock(errTxTreeIn154)
	...
	b154test = cmmutil.NewBlock(badScrVal154)
	...
	b154test = cmmutil.NewBlock(badScrValS154)
	...
	b154test = cmmutil.NewBlock(invalMissingInsS154)
	...
	b154test = cmmutil.NewBlock(spendZeroValueIn154)
	...
	b166test := cmmutil.NewBlock(spendInvalid166)
	...
	orgAddr, _ := cmmutil.DecodeAddress("ScuQxvveKGfpG1ypt6u27F99Anf7EW3cqhq")
	pkScript, _ := txscript.GenerateSStxAddrPush(orgAddr,
		cmmutil.Amount(29702992297), 0x0000)
	...
	b166test = cmmutil.NewBlock(sstxSpendInvalid166)
	...
	pkScript, _ = txscript.GenerateSStxAddrPush(orgAddr,
		cmmutil.Amount(2345438298), 0x0000)
	txOut = wire.NewTxOut(int64(0), pkScript)
	sstxToUse166.AddTxOut(txOut)
	pkScript, _ = txscript.PayToSStxChange(orgAddr)
	txOut = wire.NewTxOut(0, pkScript)
	...
	b166test = cmmutil.NewBlock(sstxSpend2Invalid166)
	...
	b166test = cmmutil.NewBlock(sstxSpend3Invalid166)
	...
	b166test = cmmutil.NewBlock(regTxSpendStakeIn166)
...
func TestCheckBlockSanity(t *testing.T) {
	params := &chaincfg.SimNetParams
	timeSource := NewMedianTime()
	block := cmmutil.NewBlock(&badBlock)
