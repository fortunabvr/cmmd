...
import (
	...
	"github.com/CommerciumBlockchain/cmmd/chaincfg/chainec"
...
// newAddressPubKey returns a new cmmutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) cmmutil.Address {
	pubkey, err := chainec.Secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := cmmutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new cmmutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) cmmutil.Address {
	addr, err := cmmutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
		secp)
	if err != nil {
...
// newAddressScriptHash returns a new cmmutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) cmmutil.Address {
	addr, err := cmmutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the cmmutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the cmmutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := cmmutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := cmmutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := cmmutil.NewAddressSecpPubKey(decodeHex("02192d74"+
	...
	p2pkCompressed2Main, err := cmmutil.NewAddressSecpPubKey(decodeHex("03b0bd"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for Commercium this would
		// be uncompressed, but standard for Commercium is 33 byte
		...
		{(*cmmutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*cmmutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*cmmutil.AddressSecpPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := cmmutil.NewAddressSecpPubKey(decodeHex("02192d7"+
	...
	tests := []struct {
		keys      []*cmmutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*cmmutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in Commercium.
			[]*cmmutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*cmmutil.AddressSecpPubKey),
			},
			1,
