...
import (
	...
	"github.com/CommerciumBlockchain/cmmd/certgen"
	rpc "github.com/CommerciumBlockchain/cmmd/rpcclient"
)

// nodeConfig contains all the args, and data required to launch a cmmd process
...
// arguments returns an array of arguments that be used to launch the cmmd
// process.
...
// command returns the exec.Cmd which will be used to start the cmmd process.
...
// to the cmmd process that is launched via Start().
...
// as the base for the log and data directories for cmmd.
...
// start creates a new cmmd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "cmmd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running cmmd process process, and waits until it exits
...
// shutdown terminates the running cmmd process, and cleans up all
