...
import (
	...
	"github.com/CommerciumBlockchain/cmmd/cequihash"
...
func (s *SpendableOut) Amount() cmmutil.Amount {
	return s.amount
}
...
func MakeSpendableOutForTx(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      cmmutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
type Generator struct {
	...
	p2shOpTrueAddr   cmmutil.Address
...
func MakeGenerator(params *chaincfg.Params, chain ChainInterface) (Generator, error) {
	...
	p2shOpTrueAddr, err := cmmutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
// a single OP_TRUE.
func (g *Generator) P2shOpTrueAddr() cmmutil.Address {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) cmmutil.Amount {
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy cmmutil.Amount, blockHeight uint32, numVotes uint16) cmmutil.Amount {
	powProportion := cmmutil.Amount(g.params.WorkRewardProportion)
	totalProportions := cmmutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := cmmutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * cmmutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) cmmutil.Amount {
	if int64(heightVotedOn+1) < g.params.StakeValidationHeight {
		return 0
	}

	fullSubsidy := g.calcFullSubsidy(heightVotedOn)
	posProportion := cmmutil.Amount(g.params.StakeRewardProportion)
	totalProportions := cmmutil.Amount(g.params.TotalSubsidyProportions())
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, powSubsidy cmmutil.Amount) {
...
func PurchaseCommitmentScript(addr cmmutil.Address, amount, voteFeeLimit, revocationFeeLimit cmmutil.Amount) []byte {
...
func (g *Generator) createTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee cmmutil.Amount) *wire.MsgTx {
...
func (g *Generator) createVoteTx(parentBlock *wire.MsgBlock, ticket *stakeTicket) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / cmmutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := cmmutil.Amount(ticket.tx.TxOut[0].Value)
...
// children nodes.  A diagram depicting how this works for Commercium transactions
...
// will be nil.
func buildMerkleTreeStore(transactions []*cmmutil.Tx) []*chainhash.Hash {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*cmmutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, cmmutil.NewTx(tx))
	}
...
//
// This compact form is only used in Commercium to encode unsigned 256-bit numbers
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee cmmutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee cmmutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := cmmutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = cmmutil.Amount(2)
	...
	var stakeTreeFees cmmutil.Amount
	for txIdx, tx := range stakeTxns {
		...
		var inputSum, outputSum cmmutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += cmmutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += cmmutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount cmmutil.Amount, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	var totalSubsidy cmmutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		payoutAddr, err := cmmutil.DecodeAddress(payout.Address)
		if err != nil {
			panic(err)
		}
		...
		totalSubsidy += cmmutil.Amount(payout.Amount)
	}
