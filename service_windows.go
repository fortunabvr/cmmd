...
const (
	// svcName is the name of cmmd service.
	svcName = "cmmdsvc"
	...
	svcDisplayName = "Cmmd Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the Commercium block " +
...
// logServiceStartOfDay logs information about cmmd when the main server has
...
// cmmdService houses the main service handler which handles all service
// updates and launching cmmdMain.
type cmmdService struct{}
...
// long-running cmmdMain (which is the real meat of cmmd), handles service
// change requests, and notifies the service control manager of changes.
func (s *cmmdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start cmmdMain in a separate goroutine so the service can start
...
// installService attempts to install the cmmd service.  Typically this should
...
// removeService attempts to uninstall the cmmd service.  Typically this should
...
// startService attempts to start the cmmd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &cmmdService{})
	if err != nil {
