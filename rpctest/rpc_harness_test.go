...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt cmmutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(cmmutil.Amount(5 * cmmutil.AtomsPerCoin))
	...
	txid = genSpend(cmmutil.Amount(5000 * cmmutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(cmmjson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(cmmjson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := cmmutil.Amount(5 * 350 * cmmutil.AtomsPerCoin)
	...
	expectedBalance = cmmutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := cmmutil.Amount(50 * cmmutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := cmmutil.Amount(numMatureOutputs * 350 * cmmutil.AtomsPerCoin)
