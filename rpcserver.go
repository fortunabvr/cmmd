...
var (
	...
	// data.
	gbtCoinbaseAux = &cmmjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &cmmjson.RPCError{
		Code:    cmmjson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &cmmjson.RPCError{
		Code:    cmmjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which cmmd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of cmmwallet.
...
// not needed.
func rpcInternalError(errStr, context string) *cmmjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return cmmjson.NewRPCError(cmmjson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *cmmjson.RPCError {
	return cmmjson.NewRPCError(cmmjson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *cmmjson.RPCError {
	return cmmjson.NewRPCError(cmmjson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}

// rpcRuleError is a convenience function to convert a
// rule error to an RPC error with the appropriate code set.
func rpcRuleError(fmtStr string, args ...interface{}) *cmmjson.RPCError {
	return cmmjson.NewRPCError(cmmjson.ErrRPCMisc,
		fmt.Sprintf(fmtStr, args...))
}
...
func rpcDuplicateTxError(fmtStr string, args ...interface{}) *cmmjson.RPCError {
	return cmmjson.NewRPCError(cmmjson.ErrRPCDuplicateTx,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *cmmjson.RPCError {
	return cmmjson.NewRPCError(cmmjson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *cmmjson.RPCError {
	return cmmjson.NewRPCError(cmmjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *cmmjson.RPCError {
	return cmmjson.NewRPCError(cmmjson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *cmmjson.RPCError {
	return cmmjson.NewRPCError(cmmjson.ErrRPCMisc, message)
}
...
// These commands will be implemented in cmmwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > cmmutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, cmmutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err := cmmutil.DecodeAddress(encodedAddr)
		if err != nil {
		...
		atomic, err := cmmutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.CreateRawSStxCmd)
	...
	for i, cout := range c.COuts {
		...
		addr, err := cmmutil.DecodeAddress(cout.Addr)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			cmmutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > cmmutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, cmmutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err = cmmutil.DecodeAddress(cout.ChangeAddr)
		if err != nil {
			return nil, rpcAddressKeyError("Wrong network: %v",
				addr)
		}
...
func handleCreateRawSSGenTx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.CreateRawSSGenTxCmd)
	...
	for i, ssgenPkh := range ssgenPkhs {
		...
		if ssgenCalcAmts[i] <= 0 ||
			ssgenCalcAmts[i] > cmmutil.MaxAmount {
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.CreateRawSSRtxCmd)
	...
	var feeAmt cmmutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = cmmutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > cmmutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, cmmutil.MaxAmount)
		}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []cmmjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]cmmjson.Vin, len(mtx.TxIn))
	if blockchain.IsCoinBaseTx(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = cmmutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &cmmjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []cmmjson.Vout {

	txType := stake.DetermineTxType(mtx)
	voutList := make([]cmmjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout cmmjson.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = cmmutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = cmmjson.Float64(commitAmt.ToCoin())
		}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx, txHash string, blkIdx uint32, blkHeader *wire.BlockHeader, blkHash string, blkHeight int64, confirmations int64) (*cmmjson.TxRawResult, error) {
	mtxHex, err := messageToHex(mtx)
	if err != nil {
		return nil, err
	}
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.DecodeRawTransactionCmd)
	...
	txReply := cmmjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := cmmutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
	...
	reply := cmmjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.EstimateStakeDiffCmd)
	...
	if c.Tickets != nil {
		...
		userEstFlt := cmmutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &cmmjson.EstimateStakeDiffResult{
		Min:      cmmutil.Amount(min).ToCoin(),
		Max:      cmmutil.Amount(max).ToCoin(),
		Expected: cmmutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleExistsAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cmmjson.ExistsAddressCmd)

	// Attempt to decode the supplied address.
	addr, err := cmmutil.DecodeAddress(c.Address)
	if err != nil {
...
func handleExistsAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cmmjson.ExistsAddressesCmd)
	addresses := make([]cmmutil.Address, len(c.Addresses))
	for i := range c.Addresses {
		// Attempt to decode the supplied address.
		addr, err := cmmutil.DecodeAddress(c.Addresses[i])
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
...
func handleExistsMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.ExistsMissedTicketsCmd)

	hashes, err := cmmjson.DecodeConcatenatedHashes(c.TxHashBlob)
	if err != nil {
		return nil, err
	}
...
func handleExistsExpiredTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.ExistsExpiredTicketsCmd)
...
func handleExistsLiveTicket(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.ExistsLiveTicketCmd)
...
func handleExistsLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.ExistsLiveTicketsCmd)
...
func handleExistsMempoolTxs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.ExistsMempoolTxsCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.GetAddedNodeInfoCmd)

	// Retrieve a list of persistent (added) peers from the Commercium server
	...
	results := make([]*cmmjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, peer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		var result cmmjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = cmmjson.Bool(peer.Connected())
		...
		addrs := make([]cmmjson.GetAddedNodeInfoResultAddr, 0,
			len(ipList))
		for _, ip := range ipList {
			var addr cmmjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &cmmjson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	sbitsFloat := float64(blockHeader.SBits) / cmmutil.AtomsPerCoin
	blockReply := cmmjson.GetBlockVerboseResult{
		Hash:             c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]cmmjson.TxRawResult, len(txns))
		for i, tx := range txns {
		...
		rawSTxns := make([]cmmjson.TxRawResult, len(stxns))
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.GetBlockHashCmd)
	hash, err := s.chain.BlockHashByHeight(c.Index)
	if err != nil {
		return nil, &cmmjson.RPCError{
			Code: cmmjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.GetBlockHeaderCmd)
	...
	blockHeaderReply := cmmjson.GetBlockHeaderVerboseResult{
		Hash:             c.Hash,
		Confirmations:    confirmations,
		Version:          blockHeader.Version,
		PreviousHash:     blockHeader.PrevBlock.String(),
		...
		PoolSize:         blockHeader.PoolSize,
		Bits:             strconv.FormatInt(int64(blockHeader.Bits), 16),
		SBits:            cmmutil.Amount(blockHeader.SBits).ToCoin(),
...
func handleGetBlockSubsidy(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.GetBlockSubsidyCmd)
	...
	rep := cmmjson.GetBlockSubsidyResult{
		PoS:   pos,
		PoW:   pow,
		Total: total,
	}
...
func (state *gbtWorkState) updateBlockTemplate(s *rpcServer, useCoinbaseValue bool) error {
	...
	if template == nil || state.prevHash == nil ||
		...
		if !useCoinbaseValue && !template.ValidPayAddress {
			...
			block := cmmutil.NewBlock(template.Block)
...
// state as a cmmjson.GetBlockTemplateResult that is ready to be encoded to
// JSON and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(bm *blockManager, useCoinbaseValue bool, submitOld *bool) (*cmmjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]cmmjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		resultTx := cmmjson.GetBlockTemplateResultTx{
	...
	stransactions := make([]cmmjson.GetBlockTemplateResultTx, 0, numSTx)
	...
	reply := cmmjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *cmmjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !cfg.SimNet && s.server.ConnectedCount() == 0 {
		return nil, &cmmjson.RPCError{
			Code:    cmmjson.ErrRPCClientNotConnected,
			Message: "Commercium is not connected",
		}
	}
	...
	if currentHeight != 0 && !s.server.blockManager.IsCurrent() {
		return nil, &cmmjson.RPCError{
			Code:    cmmjson.ErrRPCClientInInitialDownload,
			Message: "Commercium is downloading blocks...",
		}
	}
...
// reasons.
// TODO Commercium pop in the new errors from blockchain cj
...
func handleGetBlockTemplateProposal(s *rpcServer, request *cmmjson.TemplateRequest) (interface{}, error) {
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cmmjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cmmjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.GetHeadersCmd)
	blockLocators, err := cmmjson.DecodeConcatenatedHashes(c.BlockLocators)
	if err != nil {
		// Already a *cmmjson.RPCError
		return nil, err
	}
	...
	return &cmmjson.GetHeadersResult{Headers: hexBlockHeaders}, nil
}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &cmmjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := cmmjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	result := cmmjson.GetMiningInfoResult{
		Blocks:           best.Height,
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &cmmjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cmmjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*cmmjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.StatsSnapshot()
		info := &cmmjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.GetRawMempoolCmd)
	...
	if c.TxType != nil {
		switch cmmjson.GetRawMempoolTxTypeCmd(*c.TxType) {
		case cmmjson.GRMRegular:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeRegular
		case cmmjson.GRMTickets:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSStx
		case cmmjson.GRMVotes:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSGen
		case cmmjson.GRMRevocations:
			filterType = new(stake.TxType)
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.GetRawTransactionCmd)
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &cmmjson.RPCError{
			Code:    cmmjson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := cmmutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := cmmutil.Amount(nextSdiff)

	sDiffResult := &cmmjson.GetStakeDifficultyResult{
...
func convertVersionMap(m map[int]int) []cmmjson.VersionCount {
	sorted := make([]cmmjson.VersionCount, 0, len(m))
	...
	for _, v := range order {
		sorted = append(sorted, cmmjson.VersionCount{Version: uint32(v),
			Count: uint32(m[v])})
	}
...
func handleGetStakeVersionInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	count := int32(1)
	c, ok := cmd.(*cmmjson.GetStakeVersionInfoCmd)
	if !ok {
	...
	result := cmmjson.GetStakeVersionInfoResult{
		CurrentHeight: snapshot.Height,
		Hash:          snapshot.Hash.String(),
		Intervals:     make([]cmmjson.VersionInterval, 0, count),
	}
	...
	for i := int32(0); i < count; i++ {
		...
		versionInterval := cmmjson.VersionInterval{
			StartHeight:  endHeight,
...
func handleGetStakeVersions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.GetStakeVersionsCmd)
	...
	result := cmmjson.GetStakeVersionsResult{
		StakeVersions: make([]cmmjson.StakeVersions, 0, len(sv)),
	}
	for _, v := range sv {
		nsv := cmmjson.StakeVersions{
			Hash:         v.Hash.String(),
			Height:       v.Height,
			BlockVersion: v.BlockVersion,
			StakeVersion: v.StakeVersion,
			Votes: make([]cmmjson.VersionBits, 0,
				len(v.Votes)),
		}
		for _, vote := range v.Votes {
			nsv.Votes = append(nsv.Votes,
				cmmjson.VersionBits{Version: vote.Version,
					Bits: vote.Bits})
		}
...
func handleGetVoteInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c, ok := cmd.(*cmmjson.GetVoteInfoCmd)
	if !ok {
		return nil, rpcInvalidError("Invalid type: %T", c)
	}
	...
	// Assemble JSON result.
	result := cmmjson.GetVoteInfoResult{
		CurrentHeight: snapshot.Height,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &cmmjson.RPCError{
				Code: cmmjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &cmmjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: confirmations,
		Value:         cmmutil.Amount(value).ToUnit(cmmutil.AmountCoin),
		Version:       int32(txVersion),
		ScriptPubKey: cmmjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleGetWorkSubmission(s *rpcServer, hexData string) (interface{}, error) {
	...
	// blockchain.
	tempBlock := cmmutil.NewBlockDeepCopy(blockInfo.msgBlock)
...
func handleLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return cmmjson.LiveTicketsResult{Tickets: ltString}, nil
}
...
func handleMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return cmmjson.MissedTicketsResult{Tickets: mtString}, nil
}
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]cmmjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]cmmjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		amountIn := cmmutil.Amount(txIn.ValueIn).ToCoin()
	...
	vinList := make([]cmmjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &cmmjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     cmmutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr cmmutil.Address, numToSkip, numRequested uint32) ([]*cmmutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cmmjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	srtList := make([]cmmjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.SendRawTransactionCmd)
...
func handleSetGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !generate {
		...
		if c.MiningAddr != nil {
			...
			miningAddr, err := cmmutil.DecodeAddress(strAddr)
			if err != nil {
...
func min(s []cmmutil.Amount) cmmutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []cmmutil.Amount) cmmutil.Amount {
	max := cmmutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []cmmutil.Amount) cmmutil.Amount {
	sum := cmmutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / cmmutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []cmmutil.Amount) cmmutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(cmmutil.AmountSorter(s))
...
func stdDev(s []cmmutil.Amount) cmmutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := cmmutil.NewAmount(math.Sqrt(v))
...
// memory pool.
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *cmmjson.FeeInfoMempool {
	txDs := s.server.txMemPool.TxDescs()
	ticketFees := make([]cmmutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (cmmutil.Amount(txD.Fee)) * 1000 /
				cmmutil.Amount(txD.Tx.MsgTx().SerializeSize())
			ticketFees = append(ticketFees, feePerKb)
		}
	}

	return &cmmjson.FeeInfoMempool{
...
// properly set.
func calcFeePerKb(tx *cmmutil.Tx) cmmutil.Amount {
	var in cmmutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += cmmutil.Amount(txIn.ValueIn)
	}
	var out cmmutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += cmmutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / cmmutil.Amount(tx.MsgTx().SerializeSize())
}

// feeInfoForBlock fetches the ticket fee information for a given tx type in a
// block.
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*cmmjson.FeeInfoBlock, error) {
	...
	txFees := make([]cmmutil.Amount, txNum)
	itr := 0
...
// from [start, end).
func ticketFeeInfoForRange(s *rpcServer, start int64, end int64, txType stake.TxType) (*cmmjson.FeeInfoWindow, error) {
	hashes, err := s.chain.HeightRange(start, end)
	if err != nil {
		return nil, err
	}

	var txFees []cmmutil.Amount
	for i := range hashes {
...
func handleTicketFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.TicketFeeInfoCmd)
	...
	var feeInfoBlocks []cmmjson.FeeInfoBlock
	blocks := uint32(0)
	if c.Blocks != nil {
		blocks = *c.Blocks
	}
	...
	var feeInfoWindows []cmmjson.FeeInfoWindow
	windows := uint32(0)
	if c.Windows != nil {
		windows = *c.Windows
	}
	...
	return &cmmjson.TicketFeeInfoResult{
...
func handleTicketsForAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.TicketsForAddressCmd)
	...
	reply := &cmmjson.TicketsForAddressResult{
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return cmmutil.Amount(vwap).ToCoin(), nil
}
...
func handleTxFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeInfoRange cmmjson.FeeInfoRange
	...
	feeInfoRange = cmmjson.FeeInfoRange{
	...
	return &cmmjson.TxFeeInfoResult{
		FeeInfoMempool: *feeInfoMempool,
		FeeInfoBlocks:  feeInfoBlocks,
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.ValidateAddressCmd)
	result := cmmjson.ValidateAddressChainResult{}
	addr, err := cmmutil.DecodeAddress(c.Address)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cmmjson.VerifyMessageCmd)
	...
	if _, ok := addr.(*cmmutil.AddressPubKeyHash); !ok {
		return nil, &cmmjson.RPCError{
			Code:    cmmjson.ErrRPCType,
	...
	var buf bytes.Buffer
	wire.WriteVarString(&buf, 0, "Commercium Signed Message:\n")
	...
	if wasCompressed {
		serializedPK = cmmPK.SerializeCompressed()
	} else {
		serializedPK = cmmPK.SerializeUncompressed()
	}
	address, err := cmmutil.NewAddressSecpPubKey(serializedPK,
		activeNetParams.Params)
	if err != nil {
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := map[string]cmmjson.VersionResult{
		"cmmdjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, cmmjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *cmmjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(cmmjson.Error); ok &&
			jerr.Code == cmmjson.ErrUnregisteredMethod {
			parsedCmd.err = cmmjson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
// type *cmmjson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion string, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *cmmjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*cmmjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return cmmjson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *cmmjson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = cmmjson.MarshalResponse("1.0", nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = cmmjson.MarshalResponse("2.0", nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = cmmjson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="cmmd RPC", realm="jsonrpc"`)
