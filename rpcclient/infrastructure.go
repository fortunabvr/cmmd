...
// Client represents a Commercium RPC client which allows easy access to the
// various RPC methods available on a Commercium RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *cmmjson.NotifyWinningTicketsCmd:
		c.ntfnState.notifyWinningTickets = true

	case *cmmjson.NotifySpentAndMissedTicketsCmd:
		c.ntfnState.notifySpentAndMissedTickets = true

	case *cmmjson.NotifyNewTicketsCmd:
		c.ntfnState.notifyNewTickets = true

	case *cmmjson.NotifyStakeDifficultyCmd:
		c.ntfnState.notifyStakeDifficulty = true

	case *cmmjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *cmmjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *cmmjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled cmmjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := cmmjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := cmmjson.MarshalCmd("1.0", id, cmd)
	if err != nil {
		return newFutureError(err)
	}
