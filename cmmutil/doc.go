...
A Block defines a Commercium block that provides easier and more efficient
...
A Tx defines a Commercium transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Commercium address.  While the
most common type is a pay-to-pubkey-hash, Commercium already supports others and
...
To decode/encode an address:
	...
	addr, err := cmmutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
