...
// BlockChain provides functions for working with the Commercium block chain.
...
type BlockChain struct {
	...
	mainchainBlockCache     map[chainhash.Hash]*cmmutil.Block
	mainchainBlockCacheSize int

	// These fields are related to checkpoint handling.  They are protected
	// by the chain lock.
	nextCheckpoint  *chaincfg.Checkpoint
	checkpointBlock *cmmutil.Block
...
// StakeVersions is a condensed form of a cmmutil.Block that is used to prevent
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *cmmutil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchMainChainBlockByHash(hash *chainhash.Hash) (*cmmutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchBlockByHash(hash *chainhash.Hash) (*cmmutil.Block, error) {
	// Check orphan cache.
	b.orphanLock.RLock()
	...
	err := b.db.View(func(dbTx database.Tx) error {
		...
		block, err = cmmutil.NewBlockFromBytes(blockBytes)
		return err
	})
...
func (b *BlockChain) pushMainChainBlockCache(block *cmmutil.Block) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block, parent *cmmutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
	...
	blockAndParent := []*cmmutil.Block{block, parent}
...
func (b *BlockChain) dropMainChainBlockCache(block *cmmutil.Block) {
	curHash := block.Hash()
	b.mainchainBlockCacheLock.Lock()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block, parent *cmmutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *cmmutil.Block, parent *cmmutil.Block) int {
...
// adding the block.
func countNumberOfTransactions(block, parent *cmmutil.Block) uint64 {
	var numTxns uint64
	if headerApprovesParent(&block.MsgBlock().Header) {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*cmmutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]spentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*cmmutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block, parent *cmmutil.Block, flags BehaviorFlags) (bool, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *cmmutil.Block, *cmmutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *cmmutil.Block, *cmmutil.Block, *UtxoViewpoint) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		mainchainBlockCache:           make(map[chainhash.Hash]*cmmutil.Block),
