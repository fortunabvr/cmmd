...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := cmmutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(cmmutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := cmmutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := cmmutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
