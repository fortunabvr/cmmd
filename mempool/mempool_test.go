...
import (
	...
	"github.com/CommerciumBlockchain/cmmd/cmmec/secp256k1"
...
type fakeChain struct {
	...
	blocks        map[chainhash.Hash]*cmmutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *cmmutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*cmmutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *cmmutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *cmmutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
// transactions.
func txOutToSpendableOut(tx *cmmutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   cmmutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*cmmutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32) (*cmmutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput cmmutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*cmmutil.Tx, error) {
	txChain := make([]*cmmutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, cmmutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := cmmutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*cmmutil.Block),
